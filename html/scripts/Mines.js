...
async function StartMinesCountDown() {
  ...
  var NewHtml = "";
  document.getElementById("MinesBoxAreas").style.gridTemplateColumns =
    "repeat(" + mines_boxes + ", 3fr)";
  document.getElementById("MinesBoxAreas").style.gridTemplateRows =
    "repeat(" + mines_boxes + ", 3fr)";
  while (i < mines_boxes * mines_boxes) {
    ...
    NewHtml +=
      '<div id="MinesBox_' +
      i +
      '" class="ThermiteBox" onclick="MinesClick(' +
      i +
      ')"><i class="fa-solid fa-question" style="top:26%;"></i></div>';
  }
...
function MinesClick(id) {
  ...
  if (elem.value) {
    ...
    if (
      mines_boxesClicked ==
      mines_boxes * mines_boxes - mines_boxes + mine_lifes_spent
    ) {
      var i = 0;
      while (i < mines_boxes * mines_boxes) {
        i += 1;
        document.getElementById("MinesBox_" + i).style.pointerEvents = "none";
        document.getElementById("MinesBox_" + i).style.backgroundColor =
          "#e4d728";
        document.getElementById("MinesBox_" + i).innerHTML =
          '<i class="fa-solid fa-star" style="top:26%;"></i>';
        mines_current_multiplier += mines_values.finished;
      }
    }
  }
  document.getElementById("CurrentMultiplier").innerHTML =
    parseFloat(mines_current_multiplier).toFixed(2) + "x";
}

function MinesChashOut() {
  $.post(
    `https://sp-minigame/CallBack`,
    JSON.stringify(parseFloat(mines_current_multiplier).toFixed(2))
  );
