...
var Cube = /** @class */ (function () {
      ...
      if (isEmptyColumn[col]) {
        ...
        for (var row = 0; row < gridRows; row++) {
          for (var shiftCol = col; shiftCol < gridCols - 1; shiftCol++) {
            ...
            (_a = [containerCopy[nextIndex], containerCopy[currentIndex]]),
              (containerCopy[currentIndex] = _a[0]),
              (containerCopy[nextIndex] = _a[1]);
          }
        }
  ...
  Cube.prototype.cubeGravity = function (idx) {
    ...
    for (var i = 0; i < row; i++) {
      ...
      (_a = [
        containerCopy[idx - gridCols * (i + 1)],
        containerCopy[idx - gridCols * i],
      ]),
        (containerCopy[idx - gridCols * i] = _a[0]),
        (containerCopy[idx - gridCols * (i + 1)] = _a[1]);
    }
  ...
  Cube.prototype.getAdjacentCubes = function (cube) {
    ...
    if (col - 1 >= 0) adjacentCubes.push(this.container.childNodes[idx - 1]);
    if (col + 1 < gridCols)
  ...
  Cube.prototype.getConnectedCubes = function () {
    ...
    while (queue.length > 0) {
      ...
      neighbors.forEach(function (neighbor) {
        if (
          !connectedCubes.has(neighbor) &&
          neighbor.classList.contains(_this.color)
        ) {
          queue.push(neighbor);
...
function helpFunct(container, queue, path) {
  ...
  while (queue.length > 0) {
    ...
    var _a = cubesUpdate(container, connectedCubes),
      updatedContainer = _a[0],
      possibleClick = _a[1]; // [container, possibleClicks
...
function cubesUpdate(container, connectedCubes) {
  ...
  connectedCubes.forEach(function (cube) {
    ...
    for (var i = 0; i < row; i++) {
      (_a = [
        container[idx - gridCols * (i + 1)],
        container[idx - gridCols * i],
      ]),
        (container[idx - gridCols * i] = _a[0]),
        (container[idx - gridCols * (i + 1)] = _a[1]);
    }
  });
  var isEmptyColumn = new Array(gridCols).fill(true);
  // Identify empty columns
    ...
    if (isEmptyColumn[col]) {
      ...
      for (var row = 0; row < gridRows; row++) {
        for (var shiftCol = col; shiftCol < gridCols - 1; shiftCol++) {
          ...
          var nextIndex = row * gridCols + shiftCol + 1;
          // Swap the current and next columns
          (_a = [container[nextIndex], container[currentIndex]]),
            (container[currentIndex] = _a[0]),
            (container[nextIndex] = _a[1]);
        }
      }
      // Adjust for the shift when checking subsequent columns
      isEmptyColumn.splice(col, 1);
...
function checkSolvable() {
  var container = document.getElementById("container");
  var containerCopy = Array.from(container.childNodes).map(function (node) {
    return node.cloneNode(true);
  }); //Deep copy of the container so modification of the DOM cubes does not affect the original container
...
function getConnectedCubes(container, cube) {
  ...
  while (queue.length > 0) {
    ...
    neighbors.forEach(function (neighbor) {
      if (
        !connectedCubes.has(neighbor) &&
        neighbor.classList.contains(cube.classList.item(1))
      ) {
        connectedCubes.add(neighbor);
        queue.push(neighbor);
      }
...
async function endGame(outcome) {
  var timerProgress = document.querySelector(".timer-progress-bar.roofrunning");
  timerProgress.style.width = "100%";
  if (outcome === "win") {
    await EndMinigame(true, "RoofRunning");
  } else {
    await EndMinigame(false, "RoofRunning");
  }
  var container = document.getElementById("container");
  container.innerHTML = "";
  // setTimeout(function () {
  //   timerProgress.style.display = "block";
  //   resetRoofRunningGame();
  // }, 2000);
}
async function resetRoofRunningGame() {
  $("#RoofRunning").show();
  await StartCountDown(3, "RoofRunning");
  roofrunning_running = true;
  var timerProgress = document.querySelector(".timer-progress-bar.roofrunning");
  timerProgress.style.transition = "width ".concat(
    totalSeconds,
    "s cubic-bezier(0.4, 1, 0.7, 0.93)"
  );
  generateCubes();
  runTimerRoof();
}
...
window.addEventListener("message", function (NUI) {
  ...
  switch (data.Type) {
    case "RoofRunning":
      resetRoofRunningGame(data.rows, data.cols, data.time);
      ...
      for (
        var _i = 0, _a = Object.entries(cssVariables);
        _i < _a.length;
        _i++
      ) {
        var _b = _a[_i],
