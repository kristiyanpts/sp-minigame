...
document.onkeyup = function (event) {
  ...
  if (skillcheck_running) {
    ...
    if (
      document.getElementById("SC_Bar_" + skillcheck_barPosition).value ==
        true &&
      String.fromCharCode(charCode) == skillcheck_correctKey.toUpperCase()
    ) {
      EndMinigame(
        true,
        "SkillCheckMinigame",
...
function StartCountDown(int, id, style2, notxt) {
  return new Promise((resolve, reject) => {
    var countdown = int;
    var txt =
      '<div class="Countdown" style="top: 30%; font-size: 3vh;">PREPARING DEVICE</div>';
    if (notxt) {
      txt = "";
    }
    playsound("countdown");
    if (style2) {
      document.getElementById(
        id
      ).innerHTML += `<div id="CountdownScreen" class="BlackBackground"><div id="CountdownText" class="Countdown2">${countdown}</div></div>`;
    } else {
      document.getElementById(
        id
      ).innerHTML += `<div id="CountdownScreen" class="BlackBackground">${txt}<div id="CountdownText" class="Countdown" style="top: 40%;">${countdown}</div></div>`;
    }
...
function EndMinigame(bool, id, NexGame, style) {
  ...
  return new Promise((resolve, reject) => {
    document.getElementById(
      id
    ).innerHTML += `<div id="ResultScreen" class="BlackBackground"><div id="ResultBanner" class="ResultBanner" style="${style}"><img id="ResultIcon" class="ResultIcon" src="./images/check.png"><div id="ResultText" class="ResultText">Success!</div></div></div>`;
    if (bool) {
      // rounds -= 1;
      ...
      setTimeout(function () {
        ...
        $.post(`https://sp-minigame/Success`);
        $("#" + id).fadeOut();
        resolve(true);
      }, 2000);
    } else {
