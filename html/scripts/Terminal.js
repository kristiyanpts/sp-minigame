...
var DudLength = 8;
var Sound = true;
let timeLeftTerminal = 10;
let totalTimeTerminal = 10;
var InfoText =
  "ROBCO INDUSTRIES (TM) TERMALINK PROTOCOL<br />ENTER PASSWORD NOW";
var Haikus = [
...
window.addEventListener("message", function (NUI) {
  ...
  switch (data.Type) {
    case "Terminal":
      ...
      AttemptsRemaining = data.retries;
      MaxAttempts = data.retries;
      timeLeftTerminal = data.time;
      totalTimeTerminal = data.time;
      $("#Terminal").show();
      break;
  }
});
...
Start = function () {
  ...
  timer = setInterval(function () {
    timeLeftTerminal -= 0.1;
    document.getElementById("timer-fill-terminal").style.width =
      (timeLeftTerminal / totalTimeTerminal) * 100 + "%";

    if (timeLeftTerminal <= 0) {
      Failure();
    }
  }, 100);
};
...
PopulateScreen = function () {
  $("#terminal").html(
    `<div id="terminal-interior"><div id="info"></div><div id="attempts"></div><div id="column1" class="column pointers"></div><div id="column2" class="column words"></div><div id="column3" class="column pointers"></div><div id="column4" class="column words"></div><div id="output"></div><div id="console">></div>
    <div id="timer-bar-terminal">
          <div id="timer-fill-terminal"></div>
        </div></div>`
  );
};
...
TogglePower = function () {
  if (Power == "on") {
    ...
    $("#terminal").css("background-image", "url('images/bg-off.png')");
    $("#terminal").html(`
    <div id="timer-bar-terminal">
      <div id="timer-fill-terminal"></div>
    </div>
    ...
    EndMinigame(false, "Terminal");
    // $("#minigame").hide();
    // $.post(`https://${GetParentResourceName()}/Close`);
  } else {
    Power = "on";
    $("#terminal-background-off").css("visibility", "hidden");
    $("#terminal").css("background-image", "url('images/bg.png')");
    Initialize();
  }
};
          ...
          if (cont.text().substr(0, cont.text().length - 1) != insert) {
            if (Sound)
              $("#audiostuff")
                ...
                .eq(
                  Math.floor(
                    Math.random() * $("#audiostuff").find("audio").length
                  )
                )[0]
                .play();
          }
  ...
  column.find(".character").click(function () {
    ...
    if ($(this).hasClass("word")) {
      ...
      if (word.toLowerCase() == Correct.toLowerCase()) {
        ...
        UpdateOutput(
          CompareWords(word, Correct) + "/" + Correct.length + " correct."
        );
        AttemptsRemaining--;
        UpdateAttempts();
...
RemoveDud = function () {
  ...
  var WordToRemove = $(
    LiveWords[Math.floor(Math.random() * LiveWords.length)]
  ).attr("data-word");
...
Failure = function () {
  ...
  $("#terminal-interior").animate(
    ...
    },
    {
      ...
      complete: function () {
        $("#terminal").html(
          "<div id='canvas'></div><div id='adminalert'><div class='character-hover alert-text'>TERMINAL LOCKED</div><br />PLEASE CONTACT AN ADMINISTRATOR</div></div>"
        );
...
Success = function () {
  ...
  $("#terminal-interior").animate(
    ...
    },
    {
      ...
      complete: function () {
        $("#terminal").html(
          "<div id='canvas'></div><div id='adminalert'><div id='msg' class='character-hover alert-text'>TERMINAL ACCESS GRANTED</div><br /><div onClick=\"WinCondition();return false;\" id='proceed' class='alert-text'>PRESS HERE TO PROCEED</div></div>"
        );
...
AddDudBrackets = function (Nodes) {
  ...
  while (i < AllBlankIndices.length) {
    if (Math.round(Math.random() + 0.25)) {
      var Brackets =
        BracketSets[Math.floor(Math.random() * BracketSets.length)];
